...
class Cart extends Component {
    ...
    handleUpdateQuantity= (event) => {
        ...
        if(this.state.cart_items.length > 1){
            ...
            let updated_item = this.state.cart_items.filter( item => item.item.product_sku === event.target.id)
        ...
        if (!Object.keys(cart_items).length) {
            ...
            checkout = 
                <Row className={classes.Cart}>
                    ...
                    <Form> 
                        {cart_items && cart_items.map( (order_item, index)=> (
                            <Row key={order_item.item.product_sku} className={classes.Cart__Items}>
